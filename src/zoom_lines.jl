...
Common attributes shared by both rectangles and lines are `strokewidth` and `strokecolor`.
`rectattrs` and `lineattrs` are used to override the attributes of the rectangles and lines separately when needed.
"""
