...
# ╔═╡ bac46dd1-4949-4997-89e3-38e3958872b0
let
	fig = Figure()
	for (i, (colorscale, label)) in [
		(identity, "Identity scale"),
		(SymLog(1), "SymLog(1)"),
		(SymLog(100), "SymLog(100)"),
		(AsinhScale(1), "Asinh(1)"),
	] |> enumerate
		_, plt = image(fig[1,i], X; colorscale, colormap=:turbo, axis=(width=300, title=label))
...
# ╔═╡ d45424f2-d43f-452b-bfaf-a6772d9451b7
let
	fig = Figure()
	for (i, (scale, label)) in [
		(identity, "Identity scale"),
		(SymLog(1), "SymLog(1)"),
		(SymLog(10), "SymLog(10)"),
		(AsinhScale(1), "Asinh(1)"),
	] |> enumerate
		xs = range(-20, 20, length=1000)
		lines(fig[1,i], xs, xs; axis=(width=300, title=label, yscale=scale))
	end
...
let
	...
	scatter(fig[1,end+1], X[1,:], axis=(yscale=log10, yticks=BaseMulTicks([1, 3]), width=300, title="BaseMulTicks"))
	scatter(fig[1,end+1], X[1,:], axis=(yscale=log10, yticks=BaseMulTicks([1, 3]), ytickformat=EngTicks(), width=300, title="BaseMulTicks + EngTicks"))
	scatter(fig[1,end+1], X[1,:], axis=(yscale=log10, yticks=BaseMulTicks([1, 3]), ytickformat=EngTicks(:symbol), width=300, title="BaseMulTicks + EngTicks(:symbol)"))
